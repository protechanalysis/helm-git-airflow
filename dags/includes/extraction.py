...
# Constants for Production S3
S3_CONN_ID = "aws_s3_logs"
S3_BUCKET = "k8-infra-log"
S3_KEY = "tmp/raw_world_bank_data.json"


def extract_world_bank_data() -> None:
    """Extract World Bank indicator data and stream straight to AWS S3."""
    all_data = []
    country_str = ";".join(ecowas_country)
    logging.info(f"Beginning data extraction for {len(ecowas_country)} countries.")
    ...
    # Dynamic JSON serialization directly to AWS S3 bucket layer
    logging.info(f"Streaming raw extracted data up to S3 bucket: {S3_BUCKET}/{S3_KEY}")
    ...
    logging.info("S3 Extraction Upload Completed Successfully.")


def transform_to_dataframe() -> pd.DataFrame:
    """Pull raw data from S3 and transform into wide-format DataFrame."""
    logging.info(f"Reading raw data from S3 bucket: {S3_BUCKET}/{S3_KEY}")

    # Pull file down securely from Object Storage memory space
    s3_hook = S3Hook(aws_conn_id=S3_CONN_ID)
    if not s3_hook.check_for_key(key=S3_KEY, bucket_name=S3_BUCKET):
        logging.error("Raw source file not found in S3. Returning empty DataFrame.")
        return pd.DataFrame()

    s3_object = s3_hook.get_key(key=S3_KEY, bucket_name=S3_BUCKET)
    raw_data = json.loads(s3_object.get()['Body'].read().decode('utf-8'))

    if not raw_data:
        logging.warning("Raw data file is empty. Returning an empty DataFrame.")
        return pd.DataFrame()

    # Process long records to wide structure mapping
    records = []
    for entry in raw_data:
        records.append({
            "country_name": entry.get("country", {}).get("value"),          
    ...
    logging.info(f"Successfully transformed data. Final shape: {df_wide.shape}")
    return df_wide